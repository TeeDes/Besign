...
const isActive = ()=>{
  ...
  const handleLogout = async ()=>{
    try{
      await newRequest.post("/auth/logout") //Sử dụng newRequest (một đối tượng có thể là instance của Axios hoặc một thư viện HTTP khác) để thực hiện yêu cầu HTTP POST đến đường dẫn "/auth/logout". Điều này giả sử rằng "/auth/logout" là đường dẫn mà máy chủ cung cấp để đăng xuất người dùng.
      localStorage.setItem("currentUser", null); //// Sau khi đăng xuất thành công, đặt giá trị "currentUser" trong localStorage thành null
      navigate("/")
    } catch(err) {
      console.log(err)
    }
            ...
            <div className="user" onClick={()=>setOpen(!open)}> 
              ...
              {open && (<div className="option"> 
                {currentUser?.isSeller && ( //nếu ng dùng là seller thì sẽ hiện những cái dưới
                  ...
                  <Link className='link' to="/profile">Trang cá nhân</Link>
