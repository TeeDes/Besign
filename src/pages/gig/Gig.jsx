...
import { Link, useParams } from "react-router-dom"; //useParams là hook từ react-router-dom để lấy các tham số từ địa chỉ URL.
import { useQuery } from "@tanstack/react-query";
import newRequest from "../../utils/newRequest";
import Reviews from "../../components/reviews/Reviews";

function Gig() {
  const { id } = useParams(); //Sử dụng hook useParams để lấy giá trị của tham số id từ địa chỉ URL.

  const { isLoading, error, data } = useQuery({ //Sử dụng useQuery để thực hiện một query và lấy thông tin về sản phẩm từ server.
    queryKey: ["gig"],
    queryFn: () =>
  ...
  const userId = data?.userId; //Lấy userId từ dữ liệu sản phẩm để sử dụng trong query tiếp theo.

  const { //Sử dụng useQuery để thực hiện một query và lấy thông tin người bán từ server. enabled được sử dụng để đảm bảo rằng query chỉ được gọi khi userId có giá trị.
