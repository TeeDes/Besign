...
import { useMutation, useQueryClient } from "@tanstack/react-query"; //useMutation được sử dụng để thực hiện mutation (gửi dữ liệu lên server) khi thêm mới sản phẩm.
import newRequest from "../../utils/newRequest";
import { useNavigate } from "react-router-dom";
import { faSpinner } from '@fortawesome/free-solid-svg-icons';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'; //faSpinner và FontAwesomeIcon là icon và component từ thư viện Font Awesome để hiển thị biểu tượng quay vòng.


const add = () => { 
  const [singleFile, setSingleFile] = useState(undefined); //singleFile là state để lưu trữ file hình bìa.
  const [files, setFiles] = useState([]);
  const [uploading, setUploading] = useState(false);
  const [uploadSuccess, setUploadSuccess] = useState(false);

  const [state, dispatch] = useReducer(gigReducer, INITIAL_STATE); //state là state chính của form được quản lý bởi reducer gigReducer.

  const handleChange = (e) =>{ //handleChange xử lý sự kiện thay đổi trường nhập liệu trong form và cập nhật state.
    dispatch({
      type:"CHANGE_INPUT", 
      payload:{name: e.target.name, value: e.target.value},
    });
  };
  const handleFeature = (e) =>{ //handleFeature xử lý sự kiện thêm mới một feature vào sản phẩm.
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Đặt lại trạng thái uploading thành false ở đây
      setUploading(false);
  
      dispatch({ type: "ADD_IMAGES", payload: { cover, images } });
      setUploadSuccess(true); // Đặt trạng thái uploadSuccess sau khi thêm ảnh
    } catch (err) {
      console.log(err);
      // Xử lý lỗi nếu có
      setUploading(false);
      setUploadSuccess(false); // Đặt lại trạng thái uploadSuccess nếu có lỗi
    }
  };
          ...
          <div className="left">
            ...
            <div className="images">
              ...
              {uploading ? (
                ...
                uploadSuccess ? "Tải thành công" : "Tải lên"
                )}
              </button>
            </div>
